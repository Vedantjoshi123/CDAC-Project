...
import { updateStudentProfile } from '../../services/studentService'; // Make sure this API exists
...
const UpdateStudent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await updateStudentProfile(user.id, data, token);
      if (res.status === 'success') {
        toast.success('Profile updated successfully!');
        navigate('/student'); // Adjust route as needed
      } else {
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <div className="flex flex-col items-center mb-6 relative">
        <div className="relative w-36 h-36">
          <div className="w-full h-full rounded-full overflow-hidden border-4 border-blue-600 shadow-md bg-gray-100 flex items-center justify-center">
            {preview ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          {['street', 'city', 'state', 'country', 'zipCode'].map((field) => (
            <div key={field}>
              ...
              <Input name={field} value={formData.address[field]} onChange={handleChange} />
            </div>
          ))}
        </div>
